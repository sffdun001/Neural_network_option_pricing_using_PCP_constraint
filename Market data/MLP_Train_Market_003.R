...
mlp_market_model <- vector(mode = "list", length = 32)
mlp_market_cv <- vector(mode = "list", length = 32)
    ...
    for(l in lambdas){
      ...
      for (cv_fold in 1:6){
        ...
        m1 <- neural_net_single(X = as.matrix(market_data[,c('StockPrice','TimeTillExpiry')]),
                   ...
                   hidden = hidden_layers[[h]], iter = 50, learning_rate = lr,
        ...
        CV_MSE = mean((t1$Yhat-market_test[, 'CallPrice'])^2)
        CV_MAE = mean(abs(t1$Yhat-market_test[, 'CallPrice']))
        CV_R2 = 1 - sum((t1$Yhat-(market_test[, 'CallPrice']))^2) /
          sum((market_test[, 'CallPrice'] - mean(market_test[, 'CallPrice']))^2)
        model_res = c(CV_MSE, CV_MAE, CV_R2)
        CV_res= rbind(CV_res, model_res)
  
        mlp_market_model[[model_no]][[i]] = m1
  
      }
      CV_Err = colMeans(CV_res)
      CV_res  = rbind(CV_res,CV_Err)
      mlp_market_cv[[model_no]] = CV_res
...
#setNames(mlp_market_model, model_names)
#setNames(mlp_market_cv, model_names)


save(mlp_market_model, file = paste('mlp_market_001_', Sys.Date(), ".RData", sep = ''))
save(mlp_market_cv, file = paste('mlp_market_cv_001_', Sys.Date(), ".RData", sep = ''))
