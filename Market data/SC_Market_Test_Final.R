...
load('mlpSC_market_final_2020-09-08.RData')
...
for (i in 1:7){
  ...
  for (k in 1:N){
    ...
    m1 = forwardprop(X =(as.matrix(cbind(1, S0/K, data$TimeTillExpiry[1]))[,,drop = F]), 
                     Y= as.matrix(1), 
                     W = model$W_C , hidden = c(4,4), activation = 'sig', 
    ...
    MLPDelta = CallDelta(X = (as.matrix(cbind(S0/K, TimeToExpiry/365))[,,drop = F]), 
                         Weights = model$W_C, hidden = c(4,4), res = m1, activation = 'sig', 
...
row.names(t1) = c('AIH8','AIM8','AIU8','AIZ8','AIH9','AIM9','Train','AIU9/Test')
row.names(t2) = c('AIH8','AIM8','AIU8','AIZ8','AIH9','AIM9','Train','AIU9/Test')
write.csv(t1, file = paste('tab_V_SC_Market_', Sys.Date(),'.csv', sep = ''))
write.csv(t2, file = paste('tab_V_SC_Imp_Market_', Sys.Date(),'.csv', sep = ''))
...
write.csv(t1, file = paste('tab_VI_SC_Market_', Sys.Date(),'.csv', sep = ''))
