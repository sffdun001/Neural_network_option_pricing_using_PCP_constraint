...
load('Heston_train_001_2020-03-10.RData')
head(df_hest_train[[1]])
max(df_hest_train[[1]]$day)
...
mlp_hest_model <- vector(mode = "list", length = 10)

for (i in 1:10){
  Data = df_hest_train[[i]]

  
  hest_data = Data[,c('StockPrice', 'TimeTillExpiry','CallPrice')]
  hest_data[,c('StockPrice','CallPrice')] = Data[,c('StockPrice','CallPrice')]/ Data$Strike
  hest_data[, 'TimeTillExpiry'] =  Data[,'TimeTillExpiry'] / 365
  
  m1 <- neural_net_single(X = as.matrix(hest_data[,c('StockPrice','TimeTillExpiry')]), 
             Y= as.matrix(hest_data[, 'CallPrice']), 
             hidden = c(4), iter = 50, learning_rate = 0.1, 
             train_size = 100, lambda = 0, activation = 'sig', output_activation = 'sig')
  
  MSE = mean((m1$Yhat-hest_data[, 'CallPrice'])^2)
  MAE = mean(abs(m1$Yhat-hest_data[, 'CallPrice']))
  R2 = 1 - sum((m1$Yhat-(hest_data[, 'CallPrice']))^2) / 
    sum((hest_data[, 'CallPrice'] - mean(hest_data[, 'CallPrice']))^2)
  print(c(MSE,MAE,R2))
  mlp_hest_model[[i]] = m1
}

save(mlp_hest_model, file = paste('mlp_hest_001_', Sys.Date(), ".RData", sep = ''))
