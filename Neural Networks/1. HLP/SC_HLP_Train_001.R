...
rm(list = ls(all = TRUE))
setwd("~/UCT Academic/Masters/Dissertation/Technical Paper/Soft Constraint")
...
{ # This function uses stoc_NN to update weights
  ...
  { 
    ...
    FP = forwardprop_soft(X = X_bar, Y= P, W = W_P, hidden = hidden, 
                    activation = activation, output_activation= output_activation, L= L, train_size = nrow(X_bar))
    PE = sum(((FC$Yhat + exp(- rf * X_bar[,(t+1)])- FP$Yhat - X_bar[,(S+1)]))^2) / nrow(X_bar)
...
mlpSC_sig_model <- vector(mode = "list", length = 10)
...
for (i in 1:10){
  ...
  m1 <- neural_net_soft(X = as.matrix(hutch_data[,c('StockPrice','TimeTillExpiry')]), 
                       C = as.matrix(hutch_data[, 'CallPrice']), P= as.matrix(hutch_data[, 'PutPrice']),
                       S = 1, t = 2, hidden = c(4), iter_lim  = 50, learning_rate = 0.1, rf = 0.04681,
  ...
  print(c(MSE,MAE,R2))
  mlpSC_sig_model[[i]] = m1
}


save(mlpSC_sig_model, file = paste('mlpSC_model_003_', Sys.Date(), ".RData", sep = ''))
