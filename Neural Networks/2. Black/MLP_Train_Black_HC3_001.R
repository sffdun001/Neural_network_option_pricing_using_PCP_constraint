...
load('Black_train_final_2020-03-28.RData')
...
mlpHC3_black_model <- vector(mode = "list", length = 10)

for (i in 1:10){
  #i =2
  Data = df_black_train[[i]]
  black_data = Data[,c('StockPrice', 'TimeTillExpiry','CallPrice', 'PutPrice')]
  black_data[,c('StockPrice','CallPrice', 'PutPrice')] = Data[,c('StockPrice','CallPrice', 'PutPrice')]/ Data$Strike
  black_data[, 'TimeTillExpiry'] =  Data[,'TimeTillExpiry'] / 365
  
  m1 <- neural_net_HC2(X = as.matrix(black_data[,c('StockPrice','TimeTillExpiry')]), 
                      C = as.matrix(black_data[, 'CallPrice']), P= as.matrix(black_data[, 'PutPrice']),
  ...
  MSE = mean((m1$Chat-black_data[, 'CallPrice'])^2)
  MAE = mean(abs(m1$Chat-black_data[, 'CallPrice']))
  R2 = 1 - sum((m1$Chat-(black_data[, 'CallPrice']))^2) / 
    sum((black_data[, 'CallPrice'] - mean(black_data[, 'CallPrice']))^2)
  print(c(MSE,MAE,R2))
  mlpHC3_black_model[[i]] = m1
  print(paste("Completed:", i, sep = " "))
}

save(mlpHC3_black_model, file = paste('mlpHC3_black_001_', Sys.Date(), ".RData", sep = ''))
...
time_seq = rep(1/365, 100)

x.1 = cbind(1,price_seq,time_seq)
test.1 = forwardprop_HC2(X = x.1, S = 1, rf = 0.0687153, t= 2, C = time_seq, P = time_seq, 
