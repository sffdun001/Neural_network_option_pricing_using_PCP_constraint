...
load('mlpHC3_black_001_2020-03-29.RData')
...
for (K in c(45000, 47500, 50000, 52500, 55000)){
  ...
  for (m in 1:10){
    ...
    for(i in 1:500){
      ...
      R2 = 1 - sum((m1$chat*(m1$chat>0)-data$CallPrice)^2) / 
        sum((data$CallPrice*(m1$chat>0) - mean(data$CallPrice))^2)
      pe[i,] = c(MSE,MAE,R2)
    }
    MLPPriceError[[m]] = pe
  }
  ...
  #min(r2min)
  write.csv(price_error, file = paste('mlpHC3_price_error_table_001_',K,'_', Sys.Date(), '.csv', sep = '')) 
  
  save(MLPPriceError, file = paste('mlpHC3_price_error_001_',K,'_', Sys.Date(), '.csv', sep = ''))
...
write.csv(round(tab_III,4), file = paste('tab_III_black_HC3 _', Sys.Date(),'.csv', sep = ''))
...
#View(tab_IV)
write.csv(tab_IV, file = paste('tab_IV_black_HC3_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_black_HC3_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
#View(tab_VI)
write.csv(tab_VI, file = paste('tab_VI_HC3_Black_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
