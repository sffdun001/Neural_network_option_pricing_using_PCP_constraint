...
load('Black_train_001_2020-03-10.RData')
head(df_black_train[[1]])

cp = df_black_train[[1]]$CallPrice[1:201] + df_black_train[[1]]$Strike[1:201]*exp(0.06871538* df_black_train[[1]]$TimeTillExpiry[1:201]/365)
pp = df_black_train[[1]]$PutPrice[1:201] + df_black_train[[1]]$StockPrice[1:201]*exp(0.06871538* df_black_train[[1]]$TimeTillExpiry[1:201]/265)


plot(y = cp, x = df_black_train[[1]]$Strike[1:201])
lines(y = cp, x = df_black_train[[1]]$Strike[1:201], type = 'l')
sqrt(df_black_train[[1]]$Volatility)
#colnames(df_black_train[[1]]) = c("StockPrice", 'TimeTillExpiry','Strike', 'Volatility', 'CallPrice', 'PutPrice', 'day')
#Load in and check training data
...
mlp_black_model <- vector(mode = "list", length = 10)

for (i in 1:10){
  #i = 1
  Data = df_black_train[[i]]

  
  black_data = Data[,c('StockPrice', 'TimeTillExpiry','CallPrice')]
  black_data[,c('StockPrice','CallPrice')] = Data[,c('StockPrice','CallPrice')]/ Data$Strike
  black_data[, 'TimeTillExpiry'] =  Data[,'TimeTillExpiry'] / 365
  
  m1 <- neural_net_single(X = as.matrix(black_data[,c('StockPrice','TimeTillExpiry')]), 
             Y= as.matrix(black_data[, 'CallPrice']), 
             hidden = c(4), iter = 50, learning_rate = 0.1, 
             train_size = 100, lambda = 0, activation = 'sig', output_activation = 'sig')
  
  MSE = mean((m1$Yhat-black_data[, 'CallPrice'])^2)
  MAE = mean(abs(m1$Yhat-black_data[, 'CallPrice']))
  R2 = 1 - sum((m1$Yhat-(black_data[, 'CallPrice']))^2) / 
    sum((black_data[, 'CallPrice'] - mean(black_data[, 'CallPrice']))^2)
  print(c(MSE,MAE,R2))
  mlp_black_model[[i]] = m1
}

save(mlp_black_model, file = paste('mlp_black_001_', Sys.Date(), ".RData", sep = ''))
...
{ #This function finds the derivative of option with respect to the underlying. 
  #X = as.matrix(black_data[,c('StockPrice','TimeTillExpiry')])
