...
load('mlpSC_black_001_2020-03-28.RData')
...
for (K in c(45000, 47500, 50000, 52500, 55000)){
  ...
  #min(r2min)
  write.csv(price_error, file = paste('mlpSC_price_error_table_001_',K,'_', Sys.Date(), '.csv', sep = '')) 
  
  save(MLPPriceError, file = paste('mlpSC_price_error_001_',K,'_', Sys.Date(), '.csv', sep = ''))
...
write.csv(round(tab_III,4), file = paste('tab_III_black_SC_Market_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_IV)
write.csv(tab_IV, file = paste('tab_IV_black_SC_Market_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_black_SC_Market_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
#View(tab_VI)
write.csv(tab_VI, file = paste('tab_VI_SC_Black_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
