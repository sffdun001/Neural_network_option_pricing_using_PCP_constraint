...
{#res is the output of the forward propagation
 #for this function only W_bar represent weight matricies that include bias weights and W is without bias weights
    ...
    delta_C[[L+1]] = ((res$chat - res$C) + (res$phat - res$P))*dsig(res$Z_C[[L+1]])  #(res$chat - res$C) + (res$phat - res$P) +
    delta_W_C[[L+1]] = t(res$A_C[[L+1]])%*%delta_C[[L+1]]
    
    delta_P[[L+1]] = ((res$chat - res$C) + (res$phat - res$P))*dsig(res$Z_P[[L+1]]) 
                           ...
                           activation = 'sig', train_size, lambda = 0, output_activation)
{
  ...
  { 
    ...
    call_error_tracker[m] = Fwd$cErr
    put_error_tracker[m] = Fwd$pErr
...
mlpHC4_market_model <- vector(mode = "list", length = 32)
mlpHC4_market_cv <- vector(mode = "list", length = 32)
    ...
    for(l in lambdas){
      for (cv_fold in 1:length(data_main)){
        ...
        forward = forwardprop_HC2(X = as.matrix(cbind(1,market_test[,c('StockPrice', 'TimeTillExpiry')])), t = 2, 
                                  ...
                                  activation = 'sig', output_activation= 'sig', L= length(hidden_layers[[h]]), 
                                  train_size = nrow(market_test))
        ...
        mlpHC4_market_model[[model_no]][[i]] = m1
      ...
      mlpHC4_market_cv[[model_no]] = CV_res
...
#setNames(mlpHC4_market_model, model_names)
#setNames(mlpHC4_market_cv, model_names)

save(mlpHC4_market_model, file = paste('mlpHC4_market_001_', Sys.Date(), ".RData", sep = ''))
save(mlpHC4_market_cv, file = paste('mlpHC4_market_cv_001_', Sys.Date(), ".RData", sep = ''))
