...
mlpHC3_hest_model <- vector(mode = "list", length = 10)
...
for (i in 1:10){
  ...
  m1 <- neural_net_HC2(X = as.matrix(hest_data[,c('StockPrice','TimeTillExpiry')]), 
  ...
  print(c(MSE,MAE,R2))
  mlpHC3_hest_model[[i]] = m1
  print(paste("Completed:", i, sep = " "))
}

save(mlpHC3_hest_model, file = paste('mlpHC3_hest_001_', Sys.Date(), ".RData", sep = ''))
