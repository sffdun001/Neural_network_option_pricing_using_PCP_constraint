...
load('mlpHC3_hest_001_2020-03-30.RData')
...
for (K in c(45000, 47500, 50000, 52500, 55000)){
  ...
  #min(r2min)
  write.csv(price_error, file = paste('mlp_HC3_hest_price_error_table_001_',K,'_', Sys.Date(), '.csv', sep = '')) 
  
  save(MLPPriceError, file = paste('mlp_HC3_hest_price_error_001_',K,'_', Sys.Date(), '.RData', sep = ''))
}
...
write.csv(round(tab_III,4), file = paste('tab_III_hest_HC3_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_IV)
write.csv(tab_IV, file = paste('tab_IV_hest_HC3_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_hest_HC3_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
#View(tab_V_Imp)
write.csv(tab_V_Imp, file = paste('tab_V_Imp_hest_HC3_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
#View(tab_VI)
write.csv(tab_VI, file = paste('tab_VI_HC3_Hest_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
row.names(tab_VI_Imp) = c('Mean' , '(SE)', 'Minimum', 'Maximum', 'B-S Imp')

for (k in 1:5){
  PreditionErr = numeric(10)
  PreditionErrBS= sqrt(mean(BSImpHedgeError[[k]][TimeToExpiry,]^2) + var(BSImpHedgeError[[k]][TimeToExpiry,]))
  for (i in 1:10){
    PreditionErr[i]= sqrt(mean(MLPHedgeError[[k]][[i]][TimeToExpiry,]^2) + var(MLPHedgeError[[k]][[i]][TimeToExpiry,]))
  }
  ...
  tab_VI_Imp[5,k] = PreditionErrBS
  
}

#View(tab_VI_Imp)
write.csv(tab_VI_Imp, file = paste('tab_VI_Imp_HC3_Hest_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
