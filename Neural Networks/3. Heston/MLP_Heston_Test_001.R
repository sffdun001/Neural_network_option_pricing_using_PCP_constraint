...
head(df_hest_test1[[1]])
min(df_hest_test1[[1]]$Implied_vol)
...
for (K in c(45000, 47500, 50000, 52500, 55000)){
  ...
  MLPPriceError_black = vector(mode = 'list', length = 10)
  ...
  for (m in 1:10){
    ...
    for(i in 1:500){
      ...
      black_price = Blacks(F0 = data$StockPrice*data$Strike, K = data$Strike,
                           t = data$TimeTillExpiry, r = 0.0687153, sd = 0.1743364)/ data$Strike
      ...
      MSE_black = mean((black_price-data$CallPrice)^2)
      MAE_black = mean(abs(black_price-data$CallPrice))
      R2_black = 1 - sum((black_price-data$CallPrice)^2) / 
        sum((data$CallPrice - mean(data$CallPrice))^2)
      pe_black[i,] = c(MSE_black,MAE_black,R2_black)
    }
    
    
    MLPPriceError[[m]] = pe
    MLPPriceError_black[[m]] = pe_black
  }
  ...
  #min(r2min)
  write.csv(price_error, file = paste('mlp_hest_price_error_table_001_',K,'_', Sys.Date(), '.csv', sep = '')) 
  
  save(MLPPriceError, file = paste('mlp_hest_price_error_001_',K,'_', Sys.Date(), '.RData', sep = ''))
  ...
  r2max = c()
  for (i in 1:10){
    r2mean = c(r2mean, mean(MLPPriceError_black[[i]][,3]))
    r2min = c(r2min, min(MLPPriceError_black[[i]][,3]))
    r2max = c(r2max, max(MLPPriceError_black[[i]][,3]))
  }
  
  price_error_black = as.data.frame(matrix(NA, nrow = 4, ncol = 1))
  colnames(price_error) = "Black"
  price_error_black[1,1] = paste(round(mean(r2mean)*100,2), '%',sep = "")
  price_error_black[2,1] = paste(round(sd(r2mean)*100,4), '%',sep = "")
  price_error_black[3,1] = paste(round(max(r2max)*100,2), '%',sep = "")
  price_error_black[4,1] = paste(round(min(r2min)*100,2), '%',sep = "")
  #min(r2min)
  write.csv(price_error_black, file = paste('mlp_hest_black_price_error_table_001_',K,'_', Sys.Date(), '.csv', sep = '')) 
  
  save(MLPPriceError_black, file = paste('mlp_hest_black_price_error_001_',K,'_', Sys.Date(), '.RData', sep = ''))
}

plot(y = black_price, x = data$TimeTillExpiry, type = 'l')
...
#df_hest_test1[[1]]$Implied_vol[which(df_hest_test1[[1]]$StockPrice == 50000 & 
#                                            df_hest_test1[[1]]$Strike == 50000  &
#                                            df_hest_test1[[1]]$TimeTillExpiry == 90)]
#0.1743364
                                    
head(df_hest_test1[[1]])
#mean of all vols: 0.1752943
...
write.csv(round(tab_III,4), file = paste('tab_III_MLP_Hest_', Sys.Date(),'.csv', sep = ''))
...
#View(tab_IV)
write.csv(tab_IV, file = paste('tab_IV_MLP_Hest', Sys.Date(),'.csv', sep = ''))
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_MLP_hest', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))

#Black constant vol vs Black Implied Vol
tab_V = as.data.frame(matrix(NA, nrow = 4, ncol = 5))
names(tab_V)  = c('K = 40','K = 45','K = 50','K = 55','K = 60')
row.names(tab_V) = c('Mean' , '(SE)', 'Minimum', 'Maximum')
...
for (k in 1:5){
  HedgeErrFrac = numeric(10)
  for (i in 1:10){
    HedgeErrFrac[i]= length(which(abs(BSHedgeError[[k]][[i]][TimeToExpiry,]) < abs(BSImpHedgeError[[k]][TimeToExpiry,])))/500
  }
  tab_V[1,k] = mean(HedgeErrFrac)
  sd = tab_V[1,k]*(1-tab_V[1,k])/sqrt(500)
  tab_V[2,k] = paste("(", round(sd, 3), ")", sep = "")
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_MLP_hest_BlackConstant_v_BlackImplied', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
#View(tab_V_Imp)
write.csv(tab_V_Imp, file = paste('tab_V_Imp_MLP_hest_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
...
for (k in 1:5){
  HedgeErrFrac = numeric(10)
  for (i in 1:10){
    HedgeErrFrac[i]= length(which(abs(BSHedgeError[[k]][TimeToExpiry,]) < abs(BSImpHedgeError[[k]][TimeToExpiry,])))/500
  }
  tab_V[1,k] = mean(HedgeErrFrac)
  sd = tab_V[1,k]*(1-tab_V[1,k])/sqrt(500)
  tab_V[2,k] = paste("(", round(sd, 3), ")", sep = "")
...
#View(tab_V)
write.csv(tab_V, file = paste('tab_V_MLP_hest_BlackConstant_v_BlackImplied_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))

#Tab VI

tab_VI = as.data.frame(matrix(NA, nrow = 5, ncol = 5))
names(tab_VI)  = c('K = 40','K = 45','K = 50','K = 55','K = 60')
row.names(tab_VI) = c('Mean' , '(SE)', 'Minimum', 'Maximum', "Black")
...
#View(tab_VI)
write.csv(tab_VI, file = paste('tab_VI_MLP_Hest_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))


######TAB VI

tab_VI_Imp = as.data.frame(matrix(NA, nrow = 5, ncol = 5))
names(tab_VI_Imp)  = c('K = 40','K = 45','K = 50','K = 55','K = 60')
row.names(tab_VI_Imp) = c('Mean' , '(SE)', 'Minimum', 'Maximum', "Black Implied")

for (k in 1:5){
  PreditionErr = numeric(10)
  PreditionErrBSImp= sqrt(mean(BSImpHedgeError[[k]][TimeToExpiry,]^2) + var(BSImpHedgeError[[k]][TimeToExpiry,]))
  
  for (i in 1:10){
    PreditionErr[i]= sqrt(mean(MLPHedgeError[[k]][[i]][TimeToExpiry,]^2) + var(MLPHedgeError[[k]][[i]][TimeToExpiry,]))
  }
  tab_VI_Imp[1,k] = mean(PreditionErr)
  sd = sd(PreditionErr)
  tab_VI_Imp[2,k] = paste("(", round(sd, 3), ")", sep = "")
  tab_VI_Imp[3,k] = min(PreditionErr)
  tab_VI_Imp[4,k] = max(PreditionErr)
  tab_VI_Imp[5,k] = PreditionErrBSImp
  
}

#View(tab_VI_Imp)
write.csv(tab_VI_Imp, file = paste('tab_VI_Imp_MLP_Hest_', TimeToExpiry, "_", Sys.Date(),'.csv', sep = ''))
