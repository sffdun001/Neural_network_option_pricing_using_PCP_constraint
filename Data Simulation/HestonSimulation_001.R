...
rm(list = ls(all = TRUE))
setwd("~/UCT Academic/Masters/Dissertation/Technical Paper/Simulated Data")
...
r = log(1+mean(data$JIBA3M[which(data$i ==2)]/4)/100)*4
...
df_black_train = list()
for (i in 1:10) { 
  df_hest_train[[i]] = Heston_simulation(Ft0 = 50000, model_parameters = initial_params, r = 0.07, d = d)
  df_black_train[[i]] = Black_simulation(Ft0 = 50000, model_parameters = sig, r = 0.07, d= d)
  print(i)
}



FuturePrice <- d1[which(d1[,3] == 50000),1]
