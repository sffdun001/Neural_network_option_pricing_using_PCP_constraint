...
mlpSC_black_model <- vector(mode = "list", length = 10)
...
for (i in 1:10){
  ...
  m1 <- neural_net_soft(X = as.matrix(black_data[,c('StockPrice','TimeTillExpiry')]), 
  ...
  print(c(MSE,MAE,R2))
  mlpSC_black_model[[i]] = m1
}


save(mlpSC_black_model, file = paste('mlpSC_black_001_', Sys.Date(), ".RData", sep = ''))
